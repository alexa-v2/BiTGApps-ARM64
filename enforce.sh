...
PROPFILES="$SYSTEM/build.prop $VENDOR/build.prop"
...
if [ -f "$VENDOR/build.prop" ]; then
    insert_line $VENDOR/build.prop "ro.control_privapp_permissions=disable" after 'DEVICE_PROVISIONED=1' 'ro.control_privapp_permissions=disable';
fi;
